...
import { PDFDocument, PDFName, PDFHexString, PDFString, PDFDict, PDFArray, PDFRawStream, PDFNumber } from 'pdf-lib/dist/pdf-lib.esm.js';
import { md5, RC4, hexToBytes, bytesToHex } from './crypto-minimal';
 ...
 */
export function computeEncryptionKey(userPassword, ownerKey, permissions, fileId) {
 ...
 * PDFSmaller.com's implementation
 */
export function computeOwnerKey(ownerPassword, userPassword) {
 ...
 * PDFSmaller.com's implementation
 */
export function computeUserKey(encryptionKey, fileId) {
 ...
 * PDFSmaller.com's implementation
 */
export function encryptObject(data, objectNum, generationNum, encryptionKey) {
 ...
 * PDFSmaller.com's implementation
 */
export function encryptStringsInObject(obj, objectNum, generationNum, encryptionKey) {
  if (!obj) return;
...
export async function decryptPDF(pdfBytes, password) {
  try {
    ...
    if (!encryptRef) {
      // Not encrypted, just return the bytes or save the doc
      return await pdfDoc.save();
    }

    const encryptDict = context.lookup(encryptRef);
    if (!encryptDict || !(encryptDict instanceof PDFDict)) {
      throw new Error('Invalid encryption dictionary');
    }
    ...
    } else if (Array.isArray(idArray) && idArray.length > 0) {
      firstId = idArray[0];
    }

    if (firstId) {
      const idString = firstId.toString();
      const hexStr = idString.replace(/^<|>$/g, '');
      fileId = hexToBytes(hexStr);
    } else {
    ...
    let encryptionKey = computeEncryptionKey(password, o, p, fileId);

    // Validate with U entry
    const computedU = computeUserKey(encryptionKey, fileId);

    // Check if computedU matches u (first 16 bytes)
    let isValid = true;
    for (let i = 0; i < 16; i++) {
      if (computedU[i] !== u[i]) {
        isValid = false;
    ...
    for (const [ref, obj] of indirectObjects) {
      ...
      if (obj instanceof PDFRawStream) {
        const streamData = obj.contents;
        // RC4 is symmetric, so encryptObject works for decryption
        const decrypted = encryptObject(streamData, objectNum, generationNum, encryptionKey);
    ...
    // Remove Encrypt dictionary from trailer
